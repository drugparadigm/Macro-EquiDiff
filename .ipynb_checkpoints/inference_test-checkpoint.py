...
def process_single_smiles(user_smiles, inter_dir, input_id):
    ...
    with open(os.path.join(run_dir, f"/home/macrocycles/{inter_dir}/input_{input_id}/cyclizer_smiles.txt"), "w") as f:
    ...
    # ADD: Create TXT of extracted linkers from SDFs
    linkers = read_linkers_from_sdf(f"/home/macrocycles/{inter_dir}/input_{input_id}/linkers.sdf")
    with open(f"{inter_dir}/input_{input_id}/extracted_linkers.txt", 'w') as f:
    ...
    with open(f"/home/macrocycles/{inter_dir}/input_{input_id}/ops.txt", 'w') as f_out:
            ...
            for linker in linkers:
                ...
                if result is not None:
                    ...
                    if result:
                        print(result)
                        f_out.write(result + '\n')
                        macro_linker_pairs.append((result, linker))
    
    # process as before (only macros go to process_ops)
    process_ops(
        f"/home/macrocycles/{inter_dir}/input_{input_id}/ops.txt",
        f"/home/macrocycles/{inter_dir}/input_{input_id}/final_macros.txt"
    )
    print(f"Macrocycles generated successfully")
    
    # filter top 5 and get corresponding linkers
    top_macros = filter_macrocycles(
        f"/home/macrocycles/{inter_dir}/input_{input_id}/final_macros.txt",
        None,  # not writing directly here
        input_smiles=user_smiles
    )
    
    # Save top 5 macros and their linkers into the same CSV
    top_pairs = [(m, l) for m, l in macro_linker_pairs if m in top_macros][:5]
    with open(f"/home/macrocycles/{inter_dir}/input_{input_id}/top_5_macros.csv", 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Macro', 'Linker'])
        writer.writerows(top_pairs)

    
    # Return the top macrocycle from filter_macrocycles
    top_macrocycle = filter_macrocycles(f"{inter_dir}/input_{input_id}/final_macros.txt", f"{inter_dir}/input_{input_id}/top_5_macrors.csv", input_smiles=user_smiles)
...
if __name__ == "__main__":
    ...
    for i, user_smiles in enumerate(smiles_list):
        inter_dir = f"testing"
        print(f"Processing SMILES {i+1}/{len(smiles_list)}: {user_smiles}")
        result = process_single_smiles(user_smiles, inter_dir, i)
        if result['Top_Macrocycle'] is not None:
            # Add input SMILES to the top macrocycle's properties
    ...
    if top_macrocycles:
        ...
        # Reorder columns to put Input_SMILES first
        cols = ['Input_SMILES'] + [col for col in top_macrocycles_df.columns if col != 'Input_SMILES']
        ...
        print("⚠️ No valid macrocycles found for any input SMILES")