...
def preprocess_data(filepath, max_length=512):
    ...
    src_tok = [regex_tokenize(s) for s in src_list]
    tgt_tok = [regex_tokenize(s) for s in tgt_list]
...
def train_model(model, dataloader, criterion, optimizer, src_vocab, tgt_vocab, epochs=1000, device='cpu'):
    model.train()
    a = -1
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 100 == 0:
            save_checkpoint(
                ...
                path=f"checkpoints/macformer_checkpoint_epoch_{epoch+1}.pth"
            )
...
def save_checkpoint(model, optimizer, epoch, loss, path="checkpoints/macformer_checkpoint.pth"):
    torch.save({
        'epoch': epoch,
...
def main():
    ...
    train_model(model, dl, criterion, optimizer, src_vocab, tgt_vocab, epochs=1000, device=device)
